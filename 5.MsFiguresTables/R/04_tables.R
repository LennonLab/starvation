...
convert_table <- function(path, label, fullwidth = FALSE, fontsize = NULL,
  ...
  c(sprintf("%% generated by R/04_tables.R from %s -- do not edit", basename(path)),
    ...
    if (fullwidth) "\\begin{fullwidth}" else NULL,
    sprintf("\\noindent\\begin{tabular*}{\\textwidth}{@{\\extracolsep\\fill}%s@{}}",
    ...
    "\\hline",
    "\\end{tabular*}",
    if (fullwidth) "\\end{fullwidth}" else NULL,
    if (length(caption)) c("\\begin{tablenotes}",
                           sprintf("\\item %s", to_tex(paste(caption, collapse = " "))),
                           "\\end{tablenotes}") else NULL,
    "\\end{table}")
}
